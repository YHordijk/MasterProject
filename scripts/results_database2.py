...
def _result2database(results):
    ...
    d['task'] = results.task
    d['reaction'] = results.reaction
    d['R1'] = results.substituents.get('R1', None)
    d['R2'] = results.substituents.get('R2', None)
    d['Rcat'] = results.substituents.get('Rcat', None)
    d['directory'] = os.path.relpath(results.path, paths.results)
    d['stationary_point'] = results.stationary_point
    d['flags'] = ' '.join(list(sorted(results.flags)))
    d['inxyz'] = results.files.get('inxyz', None)
    d['outxyz'] = results.files.get('outxyz', None)
    t = int(results.timings.get('elapsed', 0))
...
if __name__ == '__main__':
    ...
    with DatabaseManager() as dbm:
        ...
        for i in range(dbm.Ncalcs):
            ...
            sub = utility.get_sorted_dict_values(res.substituents)
            calc_name = f'{res.reaction}.{"_".join(sub)}.{res.stationary_point}'
